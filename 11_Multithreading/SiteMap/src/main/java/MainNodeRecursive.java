...
public class MainNodeRecursive extends RecursiveAction {
    private MainNode node;

    public MainNodeRecursive(MainNode node) {
        this.node = node;
    }
    ...
    protected void compute() {
        try {
            ...
            for (Element e : elements) {
                ...
                if (isCorrectUrl(childUrl)) {
                    ...
                    node.addChild(new MainNode(childUrl));
                }
            }
        ...
        for (MainNode child : node.getChildren()) {
            MainNodeRecursive task = new MainNodeRecursive(child);
            task.compute();
        }
    }
